			...
			<div class="w-full">
                ...
                <div class="flex flex-wrap gap-y-2 max-h-[50px] overflow-y-auto mb-4 w-fit">
                    {#each partitions as partition, i}
                        ...
                        <div
                            class="flex items-start pr-2 gap-x-[2px] mx-2 p-2 rounded {i === currentSlide ? "bg-green-300 text-black" : "bg-gray-900"} cursor-pointer"
                            onclick={() => changeSelectedPartition(i)}
                        >
                            <div style="background-color: {color}" class="border-black border-1 w-3 h-3 rounded-full"></div>
                            <div
                                class="flex flex-col text-[11px] font-jakarta"
                            >
