...
use crate::storage::{format, format_unallocated, mount, umount};
use duct::cmd;
...
pub fn partitioning(blueprint: &BluePrint) -> Result<(), Error> {
    ...
    if let Some(storage) = blueprint.storage.as_ref() {
        if storage.autogenerated {
            if storage.autogenerated_mode == "singleboot" {
                // // run partition format
        ...
        } else if storage.new_partition_table {
...
fn partitioning_layout_preserved(blueprint: &BluePrint) -> Result<(), Error> {
    ...
    let max_number = blueprint
        ...
        .unwrap()
        .iter()
        .max_by_key(|partition| partition.number);
        ...
        if let Some(i_mountpoint) = i_mountpoint {
            ...
            if i_mountpoint.contains("boot") {
                cmd!(
                    "mkdir",
                    "--parents",
                    format!("/tealinux-mount{}", i_mountpoint)
                )
                .run()?;
                mount(
                    i_path.as_ref().unwrap(),
                    &format!("/tealinux-mount{}", i_mountpoint),
                    None,
                )?;
            }
